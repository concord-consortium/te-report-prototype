...
// We have 4 kinds of reports: Usage Report, Session Report, Drill Down Report,
// Module Report. These are generated by 4 big functions: genUsageReport(),
// genSessionReport(), genDrillDownReport(), and genModuleReport(). Each of them
// returns a string that is the CSV formatted contents of the report.

function genSessionReport(): string {
  ...
  const columnNames: string[] = [
    ...
    "Session Launch",
    "Session Close",
    "Activities Used in this Session",
  ...
  function buildColumnNames(): string[] {
    ...
    columnDefs.forEach( (column) => {
      subColumns.forEach( (subColumn) => {
        names.push(`${column.longTitle}:${subColumn}`);
      });
    });
    return names;
  }
...
function genUsageReport(): string {
